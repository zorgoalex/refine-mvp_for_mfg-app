...
- Проекты (за feature flag `RUNTIME_CONFIG_PROJECTS`): обязательный родитель каждого заказа с кодом-префиксом (например «ФК26»); полный номер заказа `<код>-<номер>` в шапке, списке и поиске; страницы `/projects` (список/карточка), выбор проекта в форме заказа, перенос заказа между проектами и объединение проектов с идемпотентностью и аудитом; проект без заказов архивируется автоматически.
