...
- Доски статусов (за feature flag): две независимые канбан-вкладки для общего и производственного потоков заказа, фильтры, постраничная догрузка колонок и перенос статуса через backend-команды.
...
- Проекты (за feature flag `RUNTIME_CONFIG_PROJECTS`): обязательный родитель каждого заказа с кодом-префиксом (например «ФК26»); полный номер заказа `<код>-<номер>` в шапке, списке и поиске; страницы `/projects` (список/карточка/создание проекта без заказа), выбор проекта в форме заказа, перенос заказа между проектами и объединение проектов с идемпотентностью и аудитом; проект, опустевший при переносе/объединении, архивируется автоматически.
- Базис-раскрой (за `RUNTIME_CONFIG_BAZIS_CUT` + `RUNTIME_CONFIG_BACKEND_CUT`): сохранённые наборы снимков деталей, добавление из просмотра/редактирования заказа, редактирование 33 полей и повторный BIFF8 `.xls` экспорт.
- Прямой переход из меню ERP в портал организации «Битрикс24» с переиспользованием одной вкладки.
...
- `/order-status-board` — доски общих и производственных статусов заказов.
...
- `/cut` — задания раскроя.
- `/bazis-cut` и `/bazis-cut/:id` — список и карточка наборов Базис-раскрой.
...
Runtime overlay для досок задаётся серверной переменной
`RUNTIME_CONFIG_ORDER_STATUS_BOARD=false`. Эффективный frontend-флаг также
требует включённого backend-чтения заказов (`useBackendOrdersRead`).
...
Пункт «Доски статусов» расположен сразу под производственным календарём и
открывает `/order-status-board`. На экране две независимые вкладки:
«Статусы заказов» и «Производство». Статусы оплаты показываются в карточке
только пользователям с `orders.view_financials`, но отдельной доски оплаты нет.

Доска читает проекцию `GET /api/v1/orders/status-board`. Начальная загрузка
возвращает ограниченную страницу каждой колонки; дальнейшая догрузка использует
непрозрачный keyset-курсор, привязанный к типу доски, колонке и фильтрам. Это не
заставляет браузер загружать тысячи заказов из крупных статусов. Доступные
карточки и действия ограничены backend RBAC/scope; финансовые поля маскируются
на сервере.

Перенос выполняется существующими идемпотентными командами смены общего или
производственного статуса с optimistic version. UI не делает optimistic move:
после команды доска перечитывается с backend, поэтому учитывает автоматизацию
статусов и параллельные изменения. Для производственного автостатуса ручной
перенос требует подтверждения и отключает авторасчёт по деталям.

Включение: build-time `VITE_ORDER_STATUS_BOARD=true` или runtime
`RUNTIME_CONFIG_ORDER_STATUS_BOARD=true`. Переносы дополнительно требуют
`useBackendProductionActions`; без него доска остаётся read-only.

## Переход в Битрикс24

Пункт «Битрикс24» в верхней части меню открывает портал организации
`https://mebelkz.bitrix24.kz/` в отдельной переиспользуемой вкладке. Если
пользователь уже авторизован в Битрикс24 в этом браузере, портал открывается
сразу. Иначе Битрикс24 показывает собственную форму входа.

Адрес и подпись задаются build-time переменными `VITE_BITRIX24_URL` и
`VITE_BITRIX24_LABEL`. Пустой `VITE_BITRIX24_URL` скрывает пункт. Старые
`VITE_CRM_URL`, `VITE_CRM_PATH` и `VITE_CRM_LABEL`, использовавшиеся для
Twenty, больше не управляют этим пунктом.

REST webhook и OAuth 2.0 предназначены для вызова API от имени пользователя и
не создают браузерную сессию на портале. Для входа без отдельной Bitrix-сессии
нужна корпоративная SSO-настройка на стороне Битрикс24 и общего identity
provider; client secret, webhook URL и OAuth tokens нельзя помещать во frontend.
...
Заказы удаляются мягко: кнопка «Удалить» в карточке заказа переносит заказ в
корзину (`/orders/trash`, пункт меню «Корзина»), не удаляя данные. Экран
корзины показывает номер, клиента, сумму, дату заказа, дату удаления и кто
удалил; доступен поиск по номеру. Кнопка «Восстановить» возвращает заказ; если
его номер уже занят живым заказом, backend отвечает 409 с предложенным
свободным номером и UI предлагает «Восстановить как N». Карточка удалённого
заказа открывается read-only с баннером и восстановлением.

Доступ к корзине, просмотру удалённых и восстановлению требует права
`orders.delete`. Backend: `GET /api/v1/orders?deleted=true`,
`GET /api/v1/orders/:id?includeDeleted=true`,
`POST /api/v1/orders/:id/restore` (If-Match + Idempotency-Key). Удаление и
восстановление пишут системный журнал и idempotent outbox-события
...
Расчёт раскроя выбирает движок оптимизатора автоматически: крупные группы (по умолчанию от 100 деталей, порог задаётся `BACKEND_CUT_HEURISTIC_AUTO_THRESHOLD`, `0` отключает) считаются быстрым эвристическим движком с максимальной доупаковкой, небольшие — генетическим, как раньше. В профиле параметров раскроя можно принудительно закрепить движок («Быстрый»/«Генетический») — например, отдельным профилем для крупных серийных заданий. Фактически применённый движок сохраняется в summary группы раскроя.
