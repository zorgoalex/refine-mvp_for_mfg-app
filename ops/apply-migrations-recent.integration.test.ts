...
const files = readdirSync(dir).filter((f) => /^16[4-9]_.*\.sql$/.test(f) || f === '174_mdf_execution_context.sql').sort();
...
describe.skipIf(!enabled)('migration 164-169 and 174 probes against actual SQL on PostgreSQL', () => {
  let created = false;
  beforeAll(() => {
