...
export function buildFrontendRuntimeConfig(
  ...
  const backendGroups = readBooleanEnv(
    env.RUNTIME_CONFIG_BACKEND_GROUPS,
    readBooleanEnv(env.RUNTIME_CONFIG_BACKEND_PROJECTS, false),
  );
