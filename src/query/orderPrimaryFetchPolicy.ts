...
export function orderPrimaryQueryMeta(routePath: string) {
  return {
    ...ORDER_PRIMARY_QUERY_META,
    erpPrimaryRoutePath: routePath,
  } as const;
}

export function isCurrentRouteOrderPrimaryQuery(
  query: Pick<Query, 'meta'>,
  pathname: string,
): boolean {
  return pathname !== '' && query.meta?.erpPrimaryRoutePath === pathname;
}
