import { nameRule } from '../../utils/nameRules';
import { Create } from "@refinedev/antd";
import { IResourceComponentsProps } from "@refinedev/core";
import { Form, Input, InputNumber, Checkbox } from "antd";
import { useFormWithHighlight } from "../../hooks/useFormWithHighlight";
import { StatusColorFormItem } from "../../components/StatusColor";

export const PaymentStatusCreate: React.FC<IResourceComponentsProps> = () => {
  const { formProps, saveButtonProps } = useFormWithHighlight({
    resource: "payment_statuses",
    idField: "payment_status_id",
  });

  return (
    <Create saveButtonProps={saveButtonProps}>
      <Form {...formProps} layout="vertical" initialValues={{ is_active: true, sort_order: 100 }}>
        <Form.Item label="Name" name="payment_status_name" rules={[...([{ required: true }]), nameRule("payment_status_name")]}>
          <Input />
        </Form.Item>
        <Form.Item label="Порядок сортировки" name="sort_order" rules={[{ required: true }]}>
          <InputNumber min={1} style={{ width: '100%' }} />
        </Form.Item>
        <StatusColorFormItem />
        <Form.Item label="Description" name="description">
          <Input.TextArea rows={3} />
        </Form.Item>
        <Form.Item label="Active" name="is_active" valuePropName="checked">
          <Checkbox />
        </Form.Item>
        <Form.Item label="Ref Key 1C" name="ref_key_1c">
          <Input />
        </Form.Item>
      </Form>
    </Create>
  );
};
