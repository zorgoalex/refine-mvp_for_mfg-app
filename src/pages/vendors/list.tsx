...
export const VendorList: React.FC<IResourceComponentsProps> = () => {
  ...
  const materialTypeMap = useMemo(() => {
    ...
    (materialTypesData?.data || []).forEach((t: any) => (map[t.material_type_id] = t.material_type_name));
    return map;
  }, [materialTypesData]);
    ...
    <List>
      ...
      >
        ...
        <Table.Column
          ...
          title="Тип материала"
          render={(_, record: any) => materialTypeMap[record?.material_type_id] ?? record?.material_type_id}
        />
