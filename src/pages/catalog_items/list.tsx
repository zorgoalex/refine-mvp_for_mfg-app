...
export function CatalogItemsList() {
  ...
  return <LocalizedList title="Товары и услуги" canCreate={false}>
    ...
    <Space direction="vertical" size="middle" style={{ width: '100%' }}>
      ...
      <Space wrap>
        <Input.Search aria-label="Поиск товара или услуги" placeholder="Название, артикул или 1C_key" allowClear onSearch={q => setQuery(previous => ({ ...previous, q, offset: 0 }))} style={{ width: 290 }} />
    ...
    <Modal open={open} title={editing ? (canManage ? 'Изменить позицию' : 'Карточка позиции') : 'Новая позиция'} onCancel={close} closable={!saving && !uncertain} maskClosable={false} keyboard={!saving && !uncertain} width={620} footer={<Space>
      ...
      <Form form={form} layout="vertical" disabled={!canManage || saving || uncertain} onValuesChange={() => setDirty(true)}>
        ...
        <Form.Item name="isActive" label="Активен" valuePropName="checked" extra="Выключите, чтобы архивировать. Позже позицию можно восстановить."><Switch /></Form.Item>
      </Form>
      {editing && <Descriptions title="Данные записи — заполняются системой" size="small" column={1} bordered>
        <Descriptions.Item label="ID">{editing.id}</Descriptions.Item>
        <Descriptions.Item label="Версия">{editing.version}</Descriptions.Item>
        <Descriptions.Item label="Кто создал">{`${editing.createdByName || 'Пользователь'} (#${editing.createdBy})`}</Descriptions.Item>
        <Descriptions.Item label="Кто изменил">{`${editing.editedByName || 'Пользователь'} (#${editing.editedBy})`}</Descriptions.Item>
        <Descriptions.Item label="Дата создания">{dayjs(editing.createdAt).format(DISPLAY_DATE_TIME_SECONDS_FORMAT)}</Descriptions.Item>
        <Descriptions.Item label="Дата изменения">{dayjs(editing.updatedAt).format(DISPLAY_DATE_TIME_SECONDS_FORMAT)}</Descriptions.Item>
      </Descriptions>}
    </Modal>
  </LocalizedList>;
}
