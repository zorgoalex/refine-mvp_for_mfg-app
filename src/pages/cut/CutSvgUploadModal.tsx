...
export const CutSvgUploadModal: React.FC<CutSvgUploadModalProps> = ({
  ...
  const [orderOptions, setOrderOptions] = useState<OrderOption[]>(
    defaultOrderIds.map((orderId, index) => ({
      value: orderId,
  ...
  useEffect(() => {
    ...
    setOrderOptions(defaultOrderIds.map((orderId, index) => ({
      value: orderId,
      label: defaultOrderNames[index] ? `${defaultOrderNames[index]} · #${orderId}` : `#${orderId}`,
    })));
  }, [defaultOrderIds, defaultOrderNames, open]);
  ...
  const submit = useCallback(async () => {
    ...
    const createMdfMachineFileCard = await askCreateMdfMachineFileCard();
    const idempotencyKey = createIdempotencyKey(parsed.svgContentHash);
    setSubmitting(true);
    try {
      const response = await cncTelegramApi.manualSvgUpload({
        selectedOrderIds,
      ...
      const cutJobPath = response.cutJobPath ?? (cutJobId ? `/cut?cutJobId=${cutJobId}` : null);
      Modal.success({
        title: cutJobId
