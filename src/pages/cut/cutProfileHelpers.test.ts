...
describe('describeCutProfile', () => {
  it('vacuum_table + optimal → авто text', () => {
    expect(describeCutProfile({ layout_mode: 'vacuum_table', vacuum: { direction: 'optimal' } })).toBe(
      'Вакуумный стол (авто): игнорирует текстуру плёнки, разрешает поворот деталей на 90° и сначала заполняет физическую ширину стола, затем длину.'
    );
  });

  it('vacuum_table + width → вдоль text', () => {
    expect(describeCutProfile({ layout_mode: 'vacuum_table', vacuum: { direction: 'width' } })).toBe(
      'Вакуумный стол (вдоль): запрещает поворот; сторона «высота» из списка деталей идёт вдоль длинной стороны листа. Если ширина не помещается поперёк, деталь разворачивается с предупреждением.'
    );
  });

  it('vacuum_table + height → поперёк text', () => {
    expect(describeCutProfile({ layout_mode: 'vacuum_table', vacuum: { direction: 'height' } })).toBe(
      'Вакуумный стол (поперёк): запрещает поворот; сторона «ширина» из списка деталей идёт вдоль длинной стороны листа. Если высота не помещается поперёк, деталь разворачивается с предупреждением.'
    );
  });
