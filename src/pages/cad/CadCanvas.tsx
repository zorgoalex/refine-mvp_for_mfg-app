...
export function cadPathData(path: CadPath): string {
  const cached = pathDataCache.get(path); if (cached !== undefined) return cached;
  const data = buildPathData(path); pathDataCache.set(path, data); return data;
}
function buildPathData(path: CadPath): string {
...
export function CadCanvas({ documentId, groups, sources, job, readOnly, selected, onSelect, onChange, hiddenLayers, expanded,
  ...
  const visibleSet = new Set(visible), selectedSet = new Set(selected);
  ...
  return <div className="cad-canvas-shell">
    <div className="cad-canvas-tools" role="toolbar" aria-label="Инструменты CAD">
      <Tooltip title="Выбор (V)"><Button aria-label="Выбор" icon={<SelectOutlined />} aria-pressed={tool === 'select'} type={tool === 'select' ? 'primary' : 'default'} onClick={() => setTool('select')} /></Tooltip>
      <Tooltip title="Панорама (H)"><Button aria-label="Панорама" icon={<DragOutlined />} aria-pressed={pan} type={pan ? 'primary' : 'default'} onClick={() => setTool('pan')} /></Tooltip>
      <Tooltip title="Измерить (M)"><Button aria-label="Измерить" icon={<ColumnWidthOutlined />} aria-pressed={measure} type={measure ? 'primary' : 'default'} onClick={() => { setTool('measure'); setPoints([]); }} /></Tooltip>
      <Tooltip title="Весь комплект (F)"><Button aria-label="Весь комплект" onClick={fit} icon={<AimOutlined />} /></Tooltip>
      <Tooltip title="Увеличить (+)"><Button icon={<ZoomInOutlined />} onClick={() => zoomAt(size.width / 2, size.height / 2, 1.25)} aria-label="Увеличить" /></Tooltip>
      <Tooltip title="Уменьшить (−)"><Button icon={<ZoomOutOutlined />} onClick={() => zoomAt(size.width / 2, size.height / 2, .8)} aria-label="Уменьшить" /></Tooltip>
    </div>
    <div className="cad-canvas-workspace">
    {measure && <Space className="cad-measure-tools" wrap>
    ...
    <div ref={root} className="cad-canvas" role="application" data-instance-count={groups.length} data-visible-instance-count={visible.length} tabIndex={0} aria-label="Поле CAD. V выбор, H панорама, M измерить, F весь комплект. Стрелки перемещают выбранные детали."
      onKeyDown={e => {
      ...
      <Stage width={size.width} height={size.height} x={camera.x} y={camera.y} scaleX={camera.zoom} scaleY={-camera.zoom} draggable={pan}
        ...
        <Layer>
          {groups.map(g => {
            if (!visibleSet.has(g.id)) return null;
            ...
            return <Group key={g.id} x={g.xMm} y={g.yMm} rotation={g.rotationDeg} draggable={!readOnly && !pan && !measure}
              ...
              onDragStart={() => { if (!selectedSet.has(g.id)) choose(g, false); }}
    ...
    <div className="cad-canvas-status">{camera.zoom < .01 ? (camera.zoom * 100).toFixed(2) : Math.round(camera.zoom * 100)}% · мм · Y ↑ <span>{groups.length} экземпляров · выбрано {selected.length}</span></div>
    </div>
  </div>;
}
