...
export function CadEditorPage() {
  ...
  return <div className="cad-page cad-editor">
    <header className="cad-editor-header"><h1>Фрезеровки заказов</h1><Select popupClassName="cad-compact" aria-label="Выбрать заказ" showSearch filterOption={false} onSearch={setSearch} placeholder="Найти заказ" value={orderId || undefined} options={orders.data?.data.map(o => ({ value: o.orderId, label: o.orderName }))} onChange={id => void switchTo(() => navigate(`/cad/orders/${id}`))} />
...
function CadEditorDocument({ variant, variants, orderId, active, tabKey, registerLeave, onActivate }: DocumentProps) {
  ...
  return <>
    ...
    <Drawer rootClassName="cad-compact" open={panel !== null} placement={panel === 'parts' ? 'left' : 'right'} width={Math.min(420, window.innerWidth)} title={panel === 'parts' ? 'Детали' : panel === 'properties' ? 'Свойства детали' : 'Что нужно проверить'} afterOpenChange={open => { if (open && panel === 'parts') restoreListScroll(); }} onClose={() => { if (state.incomplete) message.warning('Сначала завершите ввод параметра'); else setPanel(null); }}>
