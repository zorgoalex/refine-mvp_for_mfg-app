...
export function CadLegacyPage() {
  ...
  const activate = (id: string) => { const apply = () => { setDirty(false); setDraft(null); setVariantId(id); }; if (dirty) Modal.confirm({ className: 'cad-compact', title: 'Переключить версию без сохранения изменений?', onOk: apply }); else apply(); };
  const closeVersion = (id: string) => { const apply = () => { setOpenIds(visibleVariants.filter(v => v.id !== id).map(v => v.id)); if (variant?.id === id) { setDirty(false); setDraft(null); setVariantId(variants.find(v => v.kind === 'original')?.id ?? null); } }; if (dirty && variant?.id === id) Modal.confirm({ className: 'cad-compact', title: 'Закрыть вкладку без сохранения изменений?', content: 'Сохранённая версия останется доступна.', onOk: apply }); else apply(); };
  ...
  return <div className="cad-page">
    <header className="cad-header"><div><Typography.Title level={3}>Фрезеровки заказов</Typography.Title><Typography.Text type="secondary">Оригинал сохраняется. Рабочие версии могут объединять детали разных заказов.</Typography.Text></div>
      <Select popupClassName="cad-compact" aria-label="Выбрать заказ" showSearch filterOption={false} onSearch={setSearch} options={optionList} value={orderId || undefined} placeholder="Найти заказ" style={{ minWidth: 250 }} onChange={id => { if (dirty) { message.warning('Сначала сохраните изменения рабочей версии'); return; } navigate(`/cad/orders/${id}`); }} />
    ...
    {variant && current && <>
      ...
      <Space className="cad-toolbar" wrap>
        ...
        <Select popupClassName="cad-compact" aria-label="Открыть сохранённую версию" placeholder="Открыть версию" value={null} style={{ minWidth: 170 }} options={variants.map(v => ({ value: v.id, label: v.name }))} onChange={id => { setOpenIds(ids => [...new Set([...(ids ?? visibleVariants.map(v => v.id)), id])]); activate(id); }} />
        ...
        <Button disabled={!can('cad.export')} onClick={() => setExportOpen(true)}>Скачать фрезеровки</Button>
      </Space>
      ...
      <div className="cad-layout">
        ...
        <aside className="cad-panel"><Typography.Title level={5}>Свойства и слои</Typography.Title>
          {group && sourcePart ? <>
            ...
            <label>Фрезеровка рабочей версии<Select popupClassName="cad-compact" aria-label="Фрезеровка рабочей версии" style={{ width: '100%' }} disabled={readOnly} value={group.recipe ? `${group.recipe.code}@${group.recipe.version}` : undefined} options={catalog.data?.recipes.map(r => ({ value: `${r.code}@${r.version}`, label: `${r.display_name} · ${r.version} · ${r.status}` }))} onChange={value => { const r = catalog.data?.recipes.find(r => `${r.code}@${r.version}` === value); if (r) changeGroup({ recipe: { code: r.code, version: r.version, parameters: {} } }); }} /></label>
            ...
            {run.data?.run?.packageId && <Button disabled={!can('cad.export')} onClick={() => setExportOpen(true)}>Файлы через сводку выгрузки</Button>}
    ...
    <Modal className="cad-compact" open={cloneMode !== null} title={cloneMode === 'refresh' ? 'Новая версия по актуальным данным ERP' : 'Создать рабочую версию'} onCancel={() => setCloneMode(null)} confirmLoading={busy} onOk={() => void action(async () => { if (!variant || !cloneMode) return; const next = await cadApi.clone(variant.id, versionName, cloneMode === 'refresh'); await refresh(); setVariantId(next.id); setCloneMode(null); })}><Input aria-label="Имя версии" value={versionName} onChange={e => setVersionName(e.target.value)} /><p>Оригинал и предыдущие версии останутся без изменений.</p></Modal>
    <Modal className="cad-compact" open={importOpen} title="Добавить детали другого заказа" onCancel={() => setImportOpen(false)} onOk={importParts} okButtonProps={{ disabled: !importSource }} width={700}>
      <Space><Select popupClassName="cad-compact" aria-label="Исходный заказ" showSearch filterOption={false} onSearch={setSearch} options={optionList} value={importOrder} onChange={id => { setImportOrder(id); setImportSource(null); }} style={{ width: 300 }} /><Button loading={busy} onClick={() => void loadSource()}>Получить состав</Button></Space>
    ...
    <Modal className="cad-compact" open={mappingOpen} title="Соответствия ERP → CAD" footer={null} onCancel={() => setMappingOpen(false)} width={850}>
    ...
    {variant && <CadExportDialog open={exportOpen} onClose={() => setExportOpen(false)} flush={async () => (dirty ? await save() : variant) ?? variant} />}
  </div>;
}
