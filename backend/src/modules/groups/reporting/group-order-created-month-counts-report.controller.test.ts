...
describe('GroupOrderCreatedMonthCountsReportController', () => {
  it('fails closed when groups API is disabled', async () => {
    const controller = new GroupOrderCreatedMonthCountsReportController(service(), flags(false));
  ...
  it('returns 401 when request user is missing', async () => {
    const controller = new GroupOrderCreatedMonthCountsReportController(service(), flags(true));
  ...
  it('normalizes query and delegates with current user and request id', async () => {
    const calls: unknown[] = [];
    const controller = new GroupOrderCreatedMonthCountsReportController(
      {
  ...
  it('uses the bearerAuth swagger security scheme', () => {
    expect(Reflect.getMetadata(DECORATORS.API_SECURITY, GroupOrderCreatedMonthCountsReportController)).toEqual([
      { bearerAuth: [] },
    ]);
  });
});
