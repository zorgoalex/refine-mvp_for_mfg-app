...
import { PgGroupOrderReportRepository, UnavailableGroupOrderReportRepository } from './group-order-report.repository';

describe('PgGroupOrderReportRepository', () => {
  it('uses group_order_groups predicate against orders.order_id', async () => {
    const database = fakeDatabase();
    const repo = new PgGroupOrderReportRepository(database);
  ...
  it('supports unavailable repository fail-closed behavior', async () => {
    await expect(new UnavailableGroupOrderReportRepository().listOrderIds()).rejects.toMatchObject({
      statusCode: 503,
