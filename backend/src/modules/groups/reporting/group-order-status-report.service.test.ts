...
describe('GroupOrderStatusReportService', () => {
  it('requires both groups.view and orders.view', async () => {
    const service = new GroupOrderStatusReportService({ reports: fakeReports() });
    const query = reportQuery();
    const requiredPermissions = ['groups.view', 'orders.view'];

    await expect(service.listOrderStatusCounts({ currentUser: user(['groups.view']), query })).rejects.toMatchObject({
      statusCode: 403,
  ...
  it('delegates for users with groups.view and orders.view only', async () => {
    ...
    const service = new GroupOrderStatusReportService({ reports });
    const query = reportQuery();

    await expect(
      service.listOrderStatusCounts({ currentUser: user(['groups.view', 'orders.view']), query }),
    ).resolves.toEqual({
