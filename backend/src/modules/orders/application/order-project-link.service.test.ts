  ...
  it('notifies P8 order-link facts after a changed replace when gate is enabled', async () => {
    ...
    expect(notifications.orderCalls).toEqual([{
      ...
      facts: [
        ...
        { orderId: '15', projectId: projectId('1'), action: 'removed' },
      ],
    }]);
  });
  ...
  it('uses persisted P8 facts from an idempotent response even when current links already match', async () => {
    ...
    const service = new OrderProjectLinkService({
      links: fakeLinks({
        ...
        p8NotificationFacts: [{ orderId: '15', projectId: projectId('2'), action: 'added' }],
      }),
      projectNotifications: notifications,
      projectP8NotificationsEnabled: true,
    });
    ...
    expect(notifications.orderCalls).toEqual([{
      ...
      actorUserId: '1',
      requestId: 'request-1',
      facts: [{ orderId: '15', projectId: projectId('2'), action: 'added' }],
    }]);
...
function fakeLinks(input: {
  ...
  p8NotificationFacts?: Array<{ orderId: string; projectId: string; action: 'added' | 'removed' }>;
...
function fakeNotifications() {
  return {
    ...
    async handleProjectOrderLinksChanged(input: unknown) {
