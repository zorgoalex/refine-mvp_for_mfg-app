export interface DeadlineActionIdempotencyInput {
  ...
  sourceOrderStatusId?: string | number | null;
...
export function buildDeadlineActionIdempotencyKey(input: DeadlineActionIdempotencyInput): string {
  ...
  const sourceStatusSegment =
    input.sourceOrderStatusId === undefined || input.sourceOrderStatusId === null
      ? ''
      : `:source:${String(input.sourceOrderStatusId)}`;
  ...
  return `${input.deadlineEventId}:${input.actionType}:${targetType}:${targetId}:order:${orderId}:${actionRuleId}${sourceStatusSegment}:${targetStatusId}:${snapshotHash}`;
}
