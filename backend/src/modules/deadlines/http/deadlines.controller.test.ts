...
describe('DeadlinesController', () => {
  ...
  it('delegates create when write mode is enabled', async () => {
    const calls: string[] = [];
    ...
    const controller = createController({
      ...
      commands: {
        async create(command) {
          calls.push(`${command.currentUser.id}:${command.dto.entityType}:${command.dto.entityId}`);
          return deadline;
        },
      },
    });
    ...
    expect(calls).toEqual(['admin-id:order:42']);
  });
...
function createDeadline(): DeadlineInstanceDto {
  return {
