...
describe('mapTotalsRow', () => {
  it('maps string/number aggregate columns into a rounded CutJobTotals', () => {
    expect(mapTotalsRow({ positions: '2', details: '5', area: '4.5', sheets: '0' })).toEqual({
      positions: 2, details: 5, area: 4.5, sheets: 0,
    });
  });
  it('defaults missing/null columns to 0', () => {
    expect(mapTotalsRow({})).toEqual({ positions: 0, details: 0, area: 0, sheets: 0 });
  });
});
