...
function makeRepoForSetSheet(opts: {
  ...
  const handle = (text: string, _params: readonly unknown[]) => {
    ...
    if (sql.startsWith('SELECT cut_job_id, name, status, source, version, pdf_prewarm_state, failure_code, failure_reason, param_profile_id, sheet_material_type_id, combine_films FROM cut_job')) {
      return { rows: [{ cut_job_id: 9, name: 'J', status: opts.status, source: 'manual', version: opts.version, pdf_prewarm_state: 'pending', failure_code: null, failure_reason: null, param_profile_id: null, sheet_material_type_id: null, combine_films: false }], rowCount: 1 };
    }
