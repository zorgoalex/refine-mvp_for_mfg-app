...
function createDatabase(options: FakeDbOptions = {}) {
  ...
  const handle = (text: string, params: readonly unknown[]) => {
    ...
    if (sql.startsWith('SELECT cut_job_id, name, status, source, version, pdf_prewarm_state, params, param_profile_id, sheet_material_type_id, combine_films FROM cut_job WHERE cut_job_id = $1 FOR UPDATE')) {
      ...
      return { rows: [{ ...base, version: jobVersion, param_profile_id: options.cutJob?.param_profile_id ?? null, sheet_material_type_id: options.cutJob?.sheet_material_type_id ?? null, combine_films: options.cutJob?.combine_films ?? false }], rowCount: 1 };
    }
    ...
    // loadJob reads
    if (sql.startsWith('SELECT cut_job_id, name, status, source, version, pdf_prewarm_state, failure_code, failure_reason, param_profile_id, sheet_material_type_id, combine_films FROM cut_job WHERE cut_job_id = $1')) {
      return { rows: [{ cut_job_id: 42, name: 'J', status: 'ready', source: 'manual', version: 1, pdf_prewarm_state: 'pending', failure_code: null, failure_reason: null, param_profile_id: null, sheet_material_type_id: null, combine_films: false }], rowCount: 1 };
    }
