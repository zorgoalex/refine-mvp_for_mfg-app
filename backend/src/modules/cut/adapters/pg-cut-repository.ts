...
export class PgCutRepository implements CutRepositoryPort {
  ...
  async listJobs(query: ListCutJobsQuery): Promise<CutJobDto[]> {
    ...
    if (query.filters?.includeArchived !== true && query.filters?.status !== 'archived') {
    ...
    const result = await this.database.query<{ cut_job_id: string | number }>(
      `SELECT cut_job_id FROM cut_job${conditions.length > 0 ? ` WHERE ${conditions.join(' AND ')}` : ''} ORDER BY cut_job_id DESC LIMIT 200`,
      params,
    );
