...
import { PgProjectNotificationRecipientRepository } from './project-notification-recipient.repository';

describe('PgProjectNotificationRecipientRepository', () => {
  it('returns only current user participants for a project', async () => {
    const database = fakeDatabase();
    const repository = new PgProjectNotificationRecipientRepository(database);

    await expect(repository.listCurrentUserParticipants(projectId())).resolves.toEqual([
  ...
  it('redacts affected user member identity unless recipient has users.view', async () => {
    const repository = new PgProjectNotificationRecipientRepository(fakeDatabase({
  ...
  it('does not use project participation to grant finance, audit, deadline worker, or production command permissions', async () => {
    const repository = new PgProjectNotificationRecipientRepository(fakeDatabase({
      users: [userPermissionRow({ userId: '2', roleId: 20 })],
    }));
...
function fakeDatabase({
  ...
  return {
    ...
    async query<T>(text: string, params: readonly unknown[] = []) {
      ...
      if (text.includes('FROM public.project_participants')) return { rows: participants as T[] };
