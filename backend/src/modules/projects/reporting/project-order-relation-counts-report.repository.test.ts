...
describe('PgProjectOrderRelationCountsReportRepository', () => {
  it('uses the project_order_projects predicate and returns only relation aggregate fields', async () => {
    const queries: Array<{ sql: string; params: unknown[] }> = [];
    const repository = new PgProjectOrderRelationCountsReportRepository({
    ...
    expect(sql).toContain('JOIN public.project_order_projects pop_relation ON pop_relation.order_id = o.order_id');
    ...
    expect(sql).not.toMatch(/payment|amount|client|deadline|production_status_events|project_members|audit/i);
  });

  it('keeps counted relation rows current-only when the project scope is as-of historical', async () => {
    const queries: Array<{ sql: string; params: unknown[] }> = [];
    const repository = new PgProjectOrderRelationCountsReportRepository({
      async query(sql: string, params: unknown[]) {
        queries.push({ sql, params });
  ...
  it('keeps counted relation rows current-only when the project scope is an overlap window', async () => {
    const queries: Array<{ sql: string; params: unknown[] }> = [];
    const repository = new PgProjectOrderRelationCountsReportRepository({
      async query(sql: string, params: unknown[]) {
        queries.push({ sql, params });
  ...
  it('uses historical none-mode only for the scoped order set while counting current relation rows', async () => {
    ...
    expect(asOfSql).toContain('FROM public.project_order_projects pop_filter');
  ...
  it('fails closed when relation_type is outside the published enum', async () => {
    ...
    await expect(
      ...
      statusCode: 500,
      code: 'PROJECT_REPORT_RELATION_TYPE_INVALID',
    });
  });

  it('fails closed when the database is unavailable', async () => {
    await expect(new UnavailableProjectOrderRelationCountsReportRepository().listOrderRelationCounts()).rejects.toMatchObject({
      statusCode: 503,
