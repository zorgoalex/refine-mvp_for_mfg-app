...
import { ProjectOrderStatusReportController } from './project-order-status-report.controller';

describe('ProjectOrderStatusReportController', () => {
  it('fails closed when projects API is disabled', async () => {
    const controller = new ProjectOrderStatusReportController(service(), flags(false));
  ...
  it('returns 401 when request user is missing', async () => {
    const controller = new ProjectOrderStatusReportController(service(), flags(true));
  ...
  it('normalizes query and delegates with current user and request id', async () => {
    const calls: unknown[] = [];
    const controller = new ProjectOrderStatusReportController(
      {
