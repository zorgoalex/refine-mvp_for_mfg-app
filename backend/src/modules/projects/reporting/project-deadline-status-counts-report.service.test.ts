...
describe('ProjectDeadlineStatusCountsReportService', () => {
  it('requires projects.view orders.view and deadlines.view', async () => {
    const service = new ProjectDeadlineStatusCountsReportService({ reports: fakeReports() });
    const query = reportQuery();
    const requiredPermissions = ['projects.view', 'orders.view', 'deadlines.view'];

    await expect(
      service.listDeadlineStatusCounts({ currentUser: user(['projects.view', 'orders.view']), query }),
    ...
    await expect(service.listDeadlineStatusCounts({ currentUser: user(['deadlines.view']), query })).rejects.toMatchObject({
      statusCode: 403,
      details: { requiredPermissions, missingPermissions: ['projects.view', 'orders.view'] },
    });
  });
  ...
  it('does not treat adjacent report permissions as sufficient', async () => {
    ...
    await expect(
      service.listDeadlineStatusCounts({
        currentUser: user(['payments.view', 'deadlines.audit.view', 'projects.members.view']),
        query: reportQuery(),
      }),
    ).rejects.toMatchObject({
      statusCode: 403,
      details: {
        requiredPermissions: ['projects.view', 'orders.view', 'deadlines.view'],
        missingPermissions: ['projects.view', 'orders.view', 'deadlines.view'],
      },
    });
  });
  ...
  it('delegates for users with all report permissions', async () => {
    ...
    const service = new ProjectDeadlineStatusCountsReportService({ reports });
    const query = reportQuery();

    await expect(
      service.listDeadlineStatusCounts({
        currentUser: user(['projects.view', 'orders.view', 'deadlines.view']),
        query,
      }),
    ).resolves.toEqual({
