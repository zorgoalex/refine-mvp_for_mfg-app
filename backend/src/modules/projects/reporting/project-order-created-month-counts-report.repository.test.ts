...
describe('PgProjectOrderCreatedMonthCountsReportRepository', () => {
  it('uses orders.created_at as the fact timestamp and returns only month aggregates', async () => {
    const queries: Array<{ sql: string; params: unknown[] }> = [];
    const repository = new PgProjectOrderCreatedMonthCountsReportRepository({
    ...
    expect(sql).not.toMatch(/payment|amount|client|deadline|production_status_events|project_members|audit|order_details/i);
  ...
  it('fails closed when the database is unavailable', async () => {
    await expect(
      new UnavailableProjectOrderCreatedMonthCountsReportRepository().listOrderCreatedMonthCounts(),
  ...
  it('keeps project temporal filtering separate from the orders.created_at fact window', async () => {
    const queries: Array<{ sql: string; params: unknown[] }> = [];
    const repository = new PgProjectOrderCreatedMonthCountsReportRepository({
      async query(sql: string, params: unknown[]) {
        queries.push({ sql, params });
