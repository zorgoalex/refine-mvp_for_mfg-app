...
} as const satisfies Record<ProjectEntityTypeCode, { permission: string; from: string; where: string }>;

describe('project entity registry', () => {
  it('contains exactly the accepted allowlist', () => {
    expect(Object.keys(PROJECT_ENTITY_REGISTRY).sort()).toEqual([
      'client',
      'deadline_instance',
      'employee',
      'order',
    ...
    expect([...PROJECT_ENTITY_TYPE_CODES].sort()).toEqual(Object.keys(PROJECT_ENTITY_REGISTRY).sort());
  });
  ...
  it('defines permission metadata for every allowlisted entity type', () => {
    for (const [entityType, expected] of Object.entries(expectedRegistry)) {
      expect(PROJECT_ENTITY_REGISTRY[entityType as ProjectEntityTypeCode].requiredPermission).toBe(expected.permission);
    }
  });

  it('builds fixed existence queries with value placeholders', () => {
    for (const [entityType, expected] of Object.entries(expectedRegistry)) {
      const query = buildProjectEntityExistenceQuery(entityType as ProjectEntityTypeCode, 1);
  ...
  it('keeps request values out of SQL identifiers and text', () => {
    ...
    const query = buildProjectEntityExistenceQuery('client', maliciousId);
  ...
  it('narrows only accepted entity type codes', () => {
    expect(isProjectEntityTypeCode('client')).toBe(true);
    expect(isProjectEntityTypeCode('payment')).toBe(false);
    expect(isProjectEntityTypeCode('clients; DROP TABLE users')).toBe(false);
  });
});
