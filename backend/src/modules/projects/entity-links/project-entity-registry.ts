export interface ProjectEntityExistenceQuery {
  text: string;
  values: [string];
}

interface ProjectEntityRegistryEntry {
  readonly requiredPermission: string;
  readonly query: (entityId: string) => ProjectEntityExistenceQuery;
}

export const PROJECT_ENTITY_REGISTRY = {
  order: {
...
} as const satisfies Record<string, ProjectEntityRegistryEntry>;

export type ProjectEntityTypeCode = keyof typeof PROJECT_ENTITY_REGISTRY;

export const PROJECT_ENTITY_TYPE_CODES = Object.keys(PROJECT_ENTITY_REGISTRY) as ProjectEntityTypeCode[];

export function isProjectEntityTypeCode(value: string): value is ProjectEntityTypeCode {
  return Object.hasOwn(PROJECT_ENTITY_REGISTRY, value);
}

export function buildProjectEntityExistenceQuery(
  entityType: ProjectEntityTypeCode,
  entityId: string | number,
): ProjectEntityExistenceQuery {
  return PROJECT_ENTITY_REGISTRY[entityType].query(String(entityId));
}
