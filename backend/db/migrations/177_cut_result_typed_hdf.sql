-- Typed HDF identities in immutable cut snapshots and compact projections.
-- Forward-only: no historical snapshot rewrite or acceptance of production work.
BEGIN;

ALTER TABLE cut_result_placement
  ALTER COLUMN order_detail_id DROP NOT NULL,
  ADD COLUMN IF NOT EXISTS order_hdf_detail_id BIGINT;

ALTER TABLE cut_result_placement
  DROP CONSTRAINT IF EXISTS chk_cut_result_placement_source_only_order;
ALTER TABLE cut_result_placement
  ADD CONSTRAINT chk_cut_result_placement_source_only_order CHECK (
    (order_id IS NOT NULL AND order_detail_id IS NOT NULL AND order_detail_id > 0
      AND order_hdf_detail_id IS NULL AND item_id = 'det-' || order_detail_id::text)
    OR (order_id IS NOT NULL AND order_detail_id IS NULL AND order_hdf_detail_id IS NOT NULL
      AND order_hdf_detail_id > 0 AND item_id = 'hdf-' || order_hdf_detail_id::text)
    OR (order_detail_id IS NULL AND order_hdf_detail_id IS NULL AND item_id LIKE 'svg-%')
  );

-- Archived identities intentionally have no FK to mutable live detail rows.
CREATE INDEX IF NOT EXISTS idx_cut_result_placement_hdf_candidates
  ON cut_result_placement (order_hdf_detail_id, instance, cut_result_id DESC, variant)
  WHERE order_hdf_detail_id IS NOT NULL;

CREATE OR REPLACE FUNCTION cut_result_item_identity(p_item JSONB)
RETURNS TEXT
LANGUAGE plpgsql
IMMUTABLE
STRICT
...
DECLARE
  ...
  identity_value TEXT;
BEGIN
  -- typed_hdf_identity_v1; only untyped historical ordinary items default to det.
  IF jsonb_typeof(p_item) IS DISTINCT FROM 'object'
    OR jsonb_typeof(p_item -> 'orderId') IS DISTINCT FROM 'number'
    OR COALESCE(p_item ->> 'orderId','') !~ '^[1-9][0-9]*$'
  THEN RAISE EXCEPTION 'invalid cut item owner'; END IF;
  source_type := CASE WHEN p_item ? 'sourceType' THEN p_item ->> 'sourceType' ELSE 'order_detail' END;
  IF source_type = 'order_hdf_detail' THEN
    IF jsonb_typeof(p_item -> 'orderHdfDetailId') IS DISTINCT FROM 'number'
      OR COALESCE(p_item ->> 'orderHdfDetailId','') !~ '^[1-9][0-9]*$'
    THEN RAISE EXCEPTION 'invalid HDF cut item identity'; END IF;
    source_id := (p_item ->> 'orderHdfDetailId')::BIGINT;
    -- Existing DTOs carry a compatibility alias, NOT an ordinary detail link.
    IF COALESCE(p_item -> 'orderDetailId','null'::jsonb) <> 'null'::jsonb
      AND p_item -> 'orderDetailId' IS DISTINCT FROM p_item -> 'orderHdfDetailId'
    THEN RAISE EXCEPTION 'conflicting HDF cut item alias'; END IF;
    identity_value := 'hdf-' || source_id::TEXT;
    IF p_item ->> 'itemId' IS DISTINCT FROM identity_value
    THEN RAISE EXCEPTION 'invalid HDF cut item key'; END IF;
  ELSIF source_type = 'order_detail' THEN
    IF jsonb_typeof(p_item -> 'orderDetailId') IS DISTINCT FROM 'number'
      OR COALESCE(p_item ->> 'orderDetailId','') !~ '^[1-9][0-9]*$'
      OR COALESCE(p_item -> 'orderHdfDetailId','null'::jsonb) <> 'null'::jsonb
    THEN RAISE EXCEPTION 'invalid ordinary cut item identity'; END IF;
    source_id := (p_item ->> 'orderDetailId')::BIGINT;
    identity_value := 'det-' || source_id::TEXT;
    IF p_item ? 'itemId' AND p_item ->> 'itemId' IS DISTINCT FROM identity_value
    THEN RAISE EXCEPTION 'invalid ordinary cut item key'; END IF;
  ELSE
    RAISE EXCEPTION 'unknown cut item source type';
  END IF;
...
BEGIN
  ...
  -- typed_hdf_projection_v1: never merge det-N and hdf-N through their numeric alias.
  IF EXISTS (
    SELECT 1 FROM jsonb_array_elements(result_row.snapshot_job -> 'items') item
    GROUP BY cut_result_item_identity(item) HAVING count(*) <> 1
  ) THEN
    RAISE EXCEPTION 'cut result % has duplicate item identities', p_cut_result_id;
  END IF;

  SELECT COALESCE(
    jsonb_object_agg(cut_result_item_identity(value), value),
    '{}'::JSONB
  )
  INTO items_by_id
    ...
    LOOP
      ...
      LOOP
        ...
        INSERT INTO cut_result_placement (
          ...
          order_detail_id, order_hdf_detail_id, item_id, instance, variant, cut_group_id,
          ...
          CASE WHEN item_json ->> 'sourceType' = 'order_hdf_detail' THEN NULL
              ELSE (item_json ->> 'orderDetailId')::BIGINT END,
          CASE WHEN item_json ->> 'sourceType' = 'order_hdf_detail'
              THEN (item_json ->> 'orderHdfDetailId')::BIGINT ELSE NULL END,
          item_id_value,
