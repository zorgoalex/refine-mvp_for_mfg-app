-- Boolean "Присадка" (doweling) flag on order details.
-- Auto-set on Basis PDF import when the row note mentions "присадка".
ALTER TABLE order_details
  ADD COLUMN IF NOT EXISTS doweling boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN order_details.doweling IS 'Detail requires doweling (Присадка). Auto-set on Basis PDF import when the parsed note contains the word "присадка".';
