...
describe('139 CNC MDF original-board indexes migration', () => {
  it('adds bounded-history indexes without changing rows', () => {
    ...
    expect(runner).toContain('139_cnc_mdf_original_board_indexes*');
    expect(runner).toContain('q_idx idx_cnc_telegram_packets_mdf_original_created');
    expect(runner).toContain('q_idx idx_cut_result_original_board_created_job');
  });
});
