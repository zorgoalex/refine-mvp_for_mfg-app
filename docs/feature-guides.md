...
Боковые панели изначально свёрнуты и раскрываются независимо; на планшете список
и свойства открываются кнопками. Режим «Выбрать несколько» позволяет менять положение
группы; V/H/M/F переключают выбор, панораму, измерение и показ всего комплекта.
Стрелки перемещают выбранное, Shift увеличивает шаг.

Все экземпляры позиции показаны на общем поле отдельно: количество 10 означает
10 независимых деталей, а не один прямоугольник с подписью «×10». Каждый экземпляр
можно переместить и настроить отдельно. Значок предупреждения отмечает отличие
фрезеровки или её параметров от исходной позиции; в свойствах можно вернуть
исходную фрезеровку. Явно введённое значение по умолчанию отличием не считается.
Инструменты расположены сверху и слева, список и свойства — справа, вкладки
вариантов и состояние сохранения — снизу. Высота основных полей и кнопок —20px.

При первом открытии нового заказа экземпляры каждой позиции образуют отдельный
блок. Позиции идут справа налево, затем сверху вниз. Раскладка учитывает пропорции
видимого поля, масштаб показывает весь комплект. Между экземплярами —50 мм,
между блоками —300 мм по горизонтали и150 мм по вертикали; это расстояния
просмотра, не раскроя.
Изменение размера окна и открытие панелей не переставляют детали. Сохранённые
ручные координаты остаются прежними. Кнопка «Разложить по позициям» повторяет
раскладку под текущий размер поля в рабочем варианте; действие можно отменить.
Блок позиции не связывает перемещение его экземпляров.
Подпись экземпляра содержит номер заказа из исходного снимка (не внутренний ID),
номер позиции и номер экземпляра. Для добавленных деталей используется их заказ.

Старые варианты разворачиваются только для просмотра; открытие не изменяет
сохранённый документ. При первом редактировании или выгрузке рабочего варианта
экземпляры получают собственные сохранённые записи. Исходные позиции и количества
ERP не меняются. Старый оригинал сохраняет свой архивный формат выгрузки
с количеством в manifest; для отдельных файлов каждого экземпляра используйте
рабочий вариант. Предел рабочего варианта —5000 экземпляров, без скрытого усечения.

«Расширенный режим» раскрывает технические данные, но не расширяет права.
Библиотека фрез, рецепты и разрешённые диапазоны настраиваются в отдельном
CAD-сервисе, не на рабочем экране менеджера.
...
Статус расчёта передаётся отдельно от геометрии; список отдельных файлов
загружается страницами по100. Если комплект превышает бюджет сборки ZIP
(256MiB входных данных, 64MiB архива, 90с сборки), разделите его на рабочие варианты.
При превышении бюджета неполный ZIP не публикуется.

См. [права и флаги CAD](configuration-and-auth.md#cad) и
[порядок включения и отката редактора](deployment-and-operations.md#cad-редактор-фрезеровок).
...
изменения деталей backend пересчитывает производственный статус заказа, но
не сбрасывает ручные положения связанных файлов, БАЗИС и ванн. Ручной факт
принадлежит всей производственной карточке, а не одному заказу: каскад
«Готов»/«Выдан» или обычный возврат заказа не отменяет этот факт, в том числе
в смешанной карточке. Автоматическая терминальная колонка по полному составу
по-прежнему имеет приоритет, не удаляя сохранённое ручное положение.
Для исправления ошибочного этапа используется отдельный возврат
производственной карточки с предварительным показом последствий и
подтверждением. Ранее удалённые ручные положения автоматически не восстанавливаются.
...
Оперативная МДФ-доска запрашивает `operationalWindow=two_months`. Расчётное
окно — от выбранной даты минус два календарных месяца до выбранной даты
включительно; отсутствующее число месяца заменяется последним числом.
Например, для 13 сентября начало — 13 июля, для 30 апреля — 28 февраля
(29 февраля в високосном году). Период показа карточек выбирается отдельно:
день, неделя, две недели или месяц; по умолчанию неделя.

Наличие файлов станка или БАЗИС не ограничивает серверный отбор ванн:
актуальный результат выбирается независимо от совпадений по позициям.
Источники распила присоединяются только для расчёта готовности; отсутствие
источника даёт нулевое распиленное количество, а не удаляет карточку.
Настройка «Отображать карточки ванн без связанных файлов станка» включена
по умолчанию и управляет только видимостью на экране. Ограничения периода,
архивирования и явного скрытия остаются в силе.

За пределами периода показа, но внутри расчётного окна, файлы станка,
наборы БАЗИС и ванны передаются компактными `historicalReadinessSources`
только для заказов текущих карточек. Эти факты содержат количества позиций,
идентичность источника, колонку и сводку связанных заказов; без раскладок
и дополнительных полных карточек заказов. Полный состав смешанного источника
проверяется до отбора количеств нужного заказа. Неизвестные связи не считаются
завершением; ручные переносы и правила скрытия сохраняют приоритеты.
Общий всевременной показатель распределения БАЗИС не подмешивается в этот
расчёт (`includeBazisAllocation=false` при загрузке шапок заказов).

Поиск и «План на сегодня» меняют видимость, а не расчётные факты. Прямая
ссылка `focusBathCardId` может показать ванну вне периода показа, но не
обходит двухмесячную границу, архивирование, скрытие и выбор актуального
результата. Источники старше двух месяцев не участвуют: оперативный расчёт
не гарантирует полноту за всю историю заказа.
...
История, PDF/SVG и запросы без `operationalWindow` сохраняют прежний контракт.
Совместимый режим `operationalWindow=month` также сохранён для старых клиентов.
Откат приложения не требует удаления проекции. Новый frontend требует
`historicalReadinessSources` для двухмесячного режима и не принимает ответ
без расчётных фактов; поэтому backend обновляется первым.
