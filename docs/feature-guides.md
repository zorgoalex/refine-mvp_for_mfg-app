...
Готовность МДФ backend и frontend считают одинаково: нужен явный маркер
`МДФ`/`MDF`, а маркер другого материала в названии материала, программе,
внешнем ключе или комментариях исключает пакет из МДФ-расчёта.

Оперативная МДФ-доска запрашивает `operationalWindow=month`: 31 рабочая
календарная дата включительно, от выбранной даты минус 30 дней до конца выбранной
даты в часовом поясе БД. Старые подходящие ванны остаются карточками, пока весь
их эффективный состав не разрешён в активные детали активных производственных
заказов со статусами не ниже «Упакован». Неизвестные связи/статусы не считаются
завершением. Сначала выбирается актуальный подходящий результат задания,
затем применяется возрастное ограничение — предыдущая версия не подменяет его.

Старые физически завершённые ванны передаются только компактными
`historicalBathReadiness`: они сохраняют объём в расчёте готовности заказа,
но не создают карточки, недостающие детали или дополнительные заказы.
Фильтры заказов/связи со станком сохраняют полный состав выбранной ванны;
«План на сегодня» влияет только на видимость. Прямая ссылка передаёт
`focusBathCardId` и снимает только возрастное ограничение конкретной ванны,
не отменяя архивирование, скрытие или правила выбора результата.

Метаданные ванн читаются из `cut_result_board_projection`, без `snapshot_job`.
Миграции 149 → 150 создают проекцию/триггер и заполняют историю с проверкой
покрытия и digest; обе обязательны до обновления backend. Новые результаты
получают метаданные в транзакции сохранения. Явный исторический `isVacuum`
имеет приоритет; при отсутствии используется сохранённый `vacuum_table` в
сводках групп, не текущий префикс задания. Архивный снимок не меняется.
История, PDF/SVG и запросы без `operationalWindow` не ограничиваются месяцем.
Откат приложения не требует удаления проекции. Старый backend возвращает
полные карточки; новый ответ с ограничением обязан содержать расчётные итоги.
...
Форма читается как «Когда → Для каких заказов → Что сделать».
Условия добавляются по одному: между строками действует логическое
«И», а внутри списка статусов — «ИЛИ». Блок «Как будет работать правило»
показывает итоговую логику до сохранения; приоритет находится в
...
атомарно. Для `origin='automation'` разрешён только следующий переход к деталям:
`change_details_production_status` или
`map_order_status_to_details_production_status`. Остальные действия
отфильтровываются, поэтому статус заказа может запустить нужную МДФ-цепочку,
но циклическая автоматизация статусов заказа невозможна.
`change_production_status` материализует статус во всех активных деталях;
...
В колонке «МДФ-доска» созданная отметка ведёт прямо к связанной карточке:

- для профиля вакуумного стола — к карточке ванны;
- для остальных профилей — к карточке файла станка.

Переход доступен пользователю с `orders.view`. Если карточки ещё нет,
пользователь с `cut.manage` может принудительно создать её кнопкой «Создать
карточку» в той же ячейке. МДФ-доска открывается на рабочей дате карточки,
прокручивается к ней и визуально выделяет цель.
